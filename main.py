...
def orderLines(paths, faceClusters):
    ...
    while i < NUM_LINES:
        ...
        for j in range(i+1, NUM_LINES):
            ...
            print i, 'intersect', j, 'has size', intersect
...
def mapFromParams(prefix, facelist, timeframe, longframe, latframe):
 ...
 #   years = mat['timestamps'].flatten()
    years = io.loadmat('../data/corrected_times.mat')['years'].flatten()
...
#    faces = faces[:, ~np.all(faces == 0, axis=0)]
    ...
    # now omit ME
    faces = faces[:,1:] #XXX not necessarily
...
#    saveFeatures(websitePath + prefix + '-feats.json', faces, years, longitudes, latitudes)
    ...
    # -want to save features in the map json, so we can tie data to visualization
    # -want social graph to support overlapping clusters
    # -have to keep omitting me.... (sometimes?)
